...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
	...
	{
        if (![[[iq childElement] name] isEqualToString:@"rtc"]) {
            lastReceiveTime = [NSDate timeIntervalSinceReferenceDate];
        }
	}
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
	...
	{
        // 由于在辅导过程中会收到大量带有invisible和mainbo的type的XMPPMessage，所以在这里加一个判断，若是这两种消息，auto ping的lastReceiveTime不更新
        if (!([[message type] isEqualToString:@"invisible"] || [[message type] isEqualToString:@"mainbo"]) ) {
            lastReceiveTime = [NSDate timeIntervalSinceReferenceDate];
        }
